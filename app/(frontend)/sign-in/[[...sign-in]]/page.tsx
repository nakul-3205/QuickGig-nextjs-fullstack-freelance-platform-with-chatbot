...
// Define the Roles type for type safety
type Roles = 'client' | 'freelancer';

// 3D Animated Background Component for Sign-In
...
const AnimatedBackground3D: React.FC<AnimatedBackground3DProps> = ({ darkMode }) => {
  ...
  const count = 12000; // Slightly fewer particles than signup for subtle difference
  const positions = new Float32Array(count * 3);
  const tempVector = new THREE.Vector3();
  const gridSize = 2.0; // Size of the cube grid
  ...
  useFrame((state, delta) => {
    ...
    if (ref.current) {
      ref.current.rotation.x += delta * 0.018; // Slightly faster rotation than signup
      ref.current.rotation.y += delta * 0.022;
    }

    // Subtle camera movement based on mouse and time
    camera.position.x += (mouse.current.x * 0.15 - camera.position.x) * 0.05; // Slightly less mouse influence
    camera.position.y += (mouse.current.y * 0.15 - camera.position.y) * 0.05;
    camera.position.z = 5 + Math.cos(state.clock.elapsedTime * 0.07) * 0.6; // Gentle zoom in/out, using cos for different phase
    camera.lookAt(0, 0, 0);
    ...
    for (let i = 0; i < count; i++) {
      ...
      positionsArray[i * 3 + 1] = originalY + Math.sin(state.clock.elapsedTime * 4 + originalX * 2 + originalZ * 2) * 0.04; // Slightly different wave
      positionsArray[i * 3] = originalX + Math.cos(state.clock.elapsedTime * 5 + originalY * 3) * 0.02;
      positionsArray[i * 3 + 2] = originalZ + Math.sin(state.clock.elapsedTime * 7 + originalY * 2) * 0.03;
    }
    ref.current.geometry.attributes.position.needsUpdate = true;
  });

  // Dynamic particle color based on dark mode (matching new theme)
  const particleColor = darkMode ? new THREE.Color('#818cf8') : new THREE.Color('#34d399'); // Blue in dark, green in light
  ...
  const signInWithGoogle = async () => {
    ...
    try {
      await signIn.authenticateWithRedirect({
        ...
        redirectUrl: '/sso-callback', // Your Clerk SSO callback route
        redirectUrlComplete: '/select-role', // This will redirect to select-role after Google auth
      });
    } catch (err: any) {
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-blue-50 dark:bg-gray-950 text-gray-800 dark:text-gray-100 transition-colors duration-300">
      ...
      <motion.div
        ...
        style={{
          ...
          '--tw-gradient-from': 'rgba(52, 211, 153, 0.9)', // emerald-400 with opacity
          '--tw-gradient-to': 'rgba(59, 130, 246, 0.9)', // blue-500 with opacity
      ...
      >
        <div className="relative z-10 text-white text-center p-4">
          ...
          >
            Your gateway to seamless collaborations and rewarding opportunities.
          </motion.p>
          <div className="space-y-4">
            ...
            >
              <Briefcase size={24} className="mr-3" /> Find Your Next Gig
            </motion.div>
            <motion.div
              className="flex items-center justify-center text-lg font-semibold"
              initial={{ opacity: 0, x: 20 }}
            ...
            >
              <TrendingUp size={24} className="mr-3" /> Elevate Your Career
            </motion.div>
            <motion.div
              className="flex items-center justify-center text-lg font-semibold"
              initial={{ opacity: 0, x: -20 }}
            ...
            >
              <Zap size={24} className="mr-3" /> Streamline Your Projects
            </motion.div>
          </div>
        </div>
